...
The test will prompt you for a subject id and conduct some sanity checks on your test materials.  For example, it will check whether there are enough target items and whether they are sufficiently different to be uniquely identified when sloppy spelling is tolerated.
** Results file
...
A sample output file from the Japanese operation span task can be seen [[https://github.com/tmalsburg/py-span-task/blob/master/JapaneseOperationSpan/subject1.tsv][here]].
...
To process the data of all subjects in an experiment at once, you can use the following code:
